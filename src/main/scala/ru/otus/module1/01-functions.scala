...
object functions {
  ...
  val p: Printer = str => println(str)
    ...
    */
    ...
    def filterEven(x: List[Int]): List[Int] = x.filter(isEven)

    val x = List(1,2,3,4,5,6,7,8)
    println( filterEven(x) ) // List(2, 4, 6, 8)
    ...
    */
    ...
    def filterOdd(x: List[Int]): List[Int] = x.filter(isOdd)

    val y = List(1,2,3,4,5,6,7,8)
    println( filterOdd(y) ) // List(1, 3, 5, 7)
